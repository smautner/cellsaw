...
def add_pca(adatas, dim):
    # maybe we need to check if already done?
    data = stack(adatas)
    ...
    data.obsm[] = res # TODO needs name
    return unstack(data)


def add_umap(adatas, dim):
    # maybe we need to check if already done?
    data = stack(adatas)
    # TODO we might start from pca..
    res = umap.UMAP(n_components = dim).fit_transform()
    data.obsm[] = res # TODO needs name
    return unstack(data)
